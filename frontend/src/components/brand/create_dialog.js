...
import { createBrand } from '../../actions/admin/brand';
...
const CreateDialog = ({status, loading,err, createBrand}) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
        ...
        createBrand(values, params['shopID'])
      },
  });
    ...
    <div>
      ...
      <Dialog open={show} onClose={handleClose} TransitionComponent={Transition} maxWidth="xs" fullWidth={true} PaperProps={{style: {borderRadius: "10px", textAlign: "center"}}}>
        ...
        <DialogTitle className={classes.title}>
            <FontAwesomeIcon icon='copyright' size='2x'  className={classes.dialogIcon}/>
        </DialogTitle>
...
export default connect(mapStateToProps, {createBrand})(CreateDialog);